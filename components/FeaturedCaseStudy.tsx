...
export default function FeaturedCaseStudy({
  ...
  return (
    ...
    >
      <article
        className={`project-detail shell ${isConcept ? "concept-detail" : ""}`}
      >
        <Link href="/#work" className="text-link">
          <ArrowLeft size={16} /> Selected work
        </Link>
        <div className="detail-heading">
          <p className="eyebrow case-type">
        ...
        <div
          className={`detail-meta ${project.area ? "detail-meta-four" : ""}`}
        >
          <div>
            <span>LOCATION</span>
        ...
        <div className="case-copy">
          ...
          <div>
            ...
            <p>{project.concept}</p>
            <h3>{isConcept ? "The exploration" : "Spatial response"}</h3>
            <p>{project.response}</p>
          </div>
        </div>
        <section
          className="case-boards"
          aria-label="Project drawings and visualisations"
        >
          <div className="board-heading">
            <div>
              <p className="eyebrow">From concept to detail</p>
            ...
            {project.pdf && (
              <a className="contact-button" href={project.pdf} download>
                Download presentation <Download size={18} />
              </a>
            )}
          </div>
          <ProjectGallery
